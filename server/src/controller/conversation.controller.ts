...
import { IConversationList, IRequest } from "../interface";
...
export const conversationController = {
   ...
   getConversations: expressAsyncHandler( async ( req: IRequest<any, any, { searchKey?: string, limit: number }>, res: Response<IConversationList> ) => {
   ...
   deleteGroupConversation: expressAsyncHandler( async ( req: IRequest<any, any, { conversationId: number, limit: number }>, res: Response<IConversationList> ) => {
      const { conversationId, limit } = req.query;
      const conversationList = await deleteGroupConversationService( conversationId, req.userId!, limit );
      res.json( conversationList );
   } ),

   leaveGroupConversation: expressAsyncHandler( async ( req: IRequest<any, any, { conversationId: number, limit: number }>, res: Response<IConversationList> ) => {
      const { conversationId, limit } = req.query;
      const conversationList = await leaveGroupConversationService( conversationId, req.userId!, limit );
      res.json( conversationList );
   } ),

   deleteConversation: expressAsyncHandler( async ( req: IRequest<any, any, { conversationId: number, limit: number }>, res: Response<IConversationList> ) => {
      const { conversationId, limit } = req.query;
      const conversationList = await deleteConversationService( conversationId, req.userId!, limit );
      res.json( conversationList );
   } ),

   kickUserFromGroupConversation: expressAsyncHandler( async ( req: IRequest<any, any, { conversationId: number, userId: number }>, res: Response<{ message: string }> ) => {
